...
const Navbar = ({ textColorClass = "text-white", marginBottomClass = "" }) => {
    const [nav, setNav] = useState(false);
    const handleClick = () => setNav(!nav);

    return (
        <nav
            className={`absolute top-0 left-0 z-50 w-full h-[80px] flex items-center justify-around px-4 bg-transparent ${textColorClass} ${marginBottomClass}`}
        >
            {/* Logo */}
            <div>
                <Link to="/">
            ...
            <ul className="hidden md:flex flex-1 items-center justify-around max-w-4xl">
                ...
                <li>
                    <Link className="font-extrabold drop-shadow-sm hover:text-[color:var(--primary)] transition-colors" to="/events">Events</Link>
                </li>
                <li>
